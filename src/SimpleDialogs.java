...
* check on the input in the content node. This is called when
the
* user clicks "OK", before closing the dialog box. It must return
a null
...
// The following code is adapted with some changes from the

JavaFX API docs.

// The idea is to consume a button click on the OK button if the

state of

// the inputNode is not valid; doing so will stop the dialog
...
* headerText for the dialog. If it is to be displayed as more
than
...
Sliders to control color components.

redSlider, greenSlider, blueSlider;

private Label hueLabel, brightnessLabel, saturationLabel, // For

displaying color component values.
...
// Make a label to show a given message shown in bold, with
...
// Adjust the GUI to a new color value, when one of the

sliders has changed.

if ( ! whichSlider.isValueChanging() ) {
return; // Don't respond to change if it was set

programmatically;

// only respond if it was set by user dragging the
