...
Label status = new Label("Status: Idle"); // Displays the page

load status.

status.setMaxWidth(Double.POSITIVE_INFINITY);
Label location = new Label("Location: (empty)"); // Displays the

current URL.

location.setMaxWidth(Double.POSITIVE_INFINITY);
TextField urlInput = new TextField(); // Where the user inputs a

URL to load.

urlInput.setMaxWidth(600);
Button loadButton = new Button("Load"); // For loading the URL in
...
Button cancelButton = new Button("Cancel"); // For canceling a

load.

cancelButton.setDisable(true); // Will be enabled only when a
...
// Add a "Close All" command only if this is not the only
...
String title = window.getTitle(); // Menu item text is the
...
final c2w_BrowserWindow win = window; // (for use in a lambda

expression)

// The event handler for this menu item will bring the

corresponding

// window to the front by calling its requestFocus()
